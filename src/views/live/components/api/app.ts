...
import {ParticipantEvent, RoomEvent} from "@/components/live/room/LiveEvents";
...
import {createAudioAnalyser, supportsAV1, supportsVP9} from "@/components/live/room/utils";
...
import {DataPacket_Kind, DisconnectReason, VideoQuality} from "@/components/live/protocol/tc_models_pb";
...
const appActions = {
    connectWithFormInput: async () => {
        ...
        if (roomOpts.publishDefaults?.videoCodec === 'av1' ||
    ...
    connectToRoom: async (
        ...
        room
            ...
            .on(RoomEvent.TrackStreamStateChanged, (pub, streamState, participant) => {
                appendLog(
                    `stream state changed for ${pub.trackSid} (${participant.identity}) to ${streamState.toString()}`,
                );
            });
    ...
    flipVideo: () => {
        ...
        if (state.isFrontFacing) {
            setButtonState('flip-video-button', 'From Camera', false);
        } else {
    ...
    enterText: () => {
        ...
        if (textField.value) {
            ...
            (<HTMLTextAreaElement>$('chat')).value += `${currentRoom.localParticipant.identity} (me): ${textField.value}\n`;
            textField.value = '';
        }
    },
...
function setButtonsForState(connected: boolean) {
    ...
    const disconnectSet = ['connect-button'];

    const toRemove = connected ? connectedSet : disconnectSet;
    const toAdd = connected ? disconnectSet : connectedSet;
...
function updateButtonsForPublishState() {
    ...
    setButtonState(
        'toggle-video-button',
        `${lp.isCameraEnabled ? 'Disabled' : 'Enable'} Video`,
        lp.isCameraEnabled,
    );
