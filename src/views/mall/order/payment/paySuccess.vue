...
// 支付结果
const paymentResult = ref(undefined);
// 支付方式
const paymentMode = ref(undefined);
...
const onClickViewOrder = () => {
  router.push({name: 'userOrder', params: {active: '0'}})
}

onMounted(() => {
  paymentResult.value = route.params.paymentResult;
  paymentMode.value = route.params.paymentMode;
  paymentAmount.value = route.params.paymentAmount;
});
</script>
