...
import {memberLogin, memberInfo} from '@/api/member'
...
const register_user = ref({
  username: undefined,
  password: undefined,
  confirmPassword: undefined
});
const syncUserInfo = (data) => {
  store.dispatch('syncUserInfo', data);
};
...
const onSubmitLogin = (values) => {
  ...
  memberLogin({
    username: login_user.value.username,
    password: login_user.value.password,
  }).then(response => {
    let token = response.data.tokenHead + response.data.token;
    syncToken(token);
    memberInfo().then(response => {
      syncUserInfo(response.data);
      router.back();
    });
  })
};
