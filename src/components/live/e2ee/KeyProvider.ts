...
import {KeyProviderCallbacks, KeyProviderEvent} from "@/components/live/e2ee/e2eeEvents";
import TypedEventEmitter from "typed-emitter";
import {KeyInfo, KeyProviderOptions} from "@/components/live/e2ee/types";
import {KEY_PROVIDER_DEFAULTS} from "@/components/live/e2ee/constants";
import log from '@/components/live/logger';
import {createKeyMaterialFromBuffer, createKeyMaterialFromString} from "@/components/live/e2ee/utils";
...
export class BaseKeyProvider extends (EventEmitter as new () => TypedEventEmitter<KeyProviderCallbacks>) {
    ...
    protected onSetEncryptionKey(key: CryptoKey, participantIdentity?: string, keyIndex?: number) {
        ...
        this.keyInfoMap.set(`${participantIdentity ?? 'shared'}-${keyInfo ?? 0}`, keyInfo);
