...
import {isSafari} from "@/components/live/room/utils";
...
export default class DeviceManager {
        ...
        if (
            ...
            if (isDummyDeviceOrEmpty) {
                const permissionsToAcquire = {
                    video: kind !== 'audiooutput' && kind !== 'audiooutput',
    ...
    async normalizeDeviceId(
        ...
        const device = devices.find((d) => (d.groupId === groupId || (groupIdCounts.get(d.groupId) ?? 0) > 1) && d.deviceId !== defaultId);
