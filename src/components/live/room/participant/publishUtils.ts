...
import {TrackInvalidError} from "@/components/live/room/errors";
import log from "@/components/live/logger";

import {
    BackupVideoCodec,
    ScreenSharePresets,
    TrackPublishOptions, VideoCodec, VideoEncoding,
    VideoPreset,
    VideoPresets,
    VideoPresets43
...
export function computeVideoEncodings(
    ...
    if (scalabilityMode && isSVCCodec(videoCodec)) {
        log.debug(`using svc width scalabilityMode ${scalabilityMode}`);
...
export function sortPresets(presets: Array<VideoPreset> | undefined) {
    ...
    return presets.sort((a, b) => {
        ...
        if (aEnc.maxBitrate === bEnc.maxBitrate && aEnc.maxBitrate && bEnc.maxBitrate) {
